...
Future<Alert> createAlerta(String enteRegulatorio, String fechaAlerta,
    String mensajeAlerta) async {
...
class AlertaPut extends StatefulWidget {
    const AlertaPut({super.key});

  // const AlertaPut({Key? key, required this.idCapturado}) : super(key: key);

  // final String idCapturado;
...
class _AlertaPutState extends State<AlertaPut> {
  ...
  void initState() {
    super.initState();
    // idCapturado = widget.idCapturado;
  }
    ...
    return Column(
      ...
      children: <Widget>[
        TextField(
          ...
          decoration:
              const InputDecoration(hintText: 'Digite Ente Regulatorio'),
        ),
        TextField(
        ...
        ElevatedButton(
          onPressed: () {
            setState(() {
              _futureAlert = createAlerta(_enteRegulatorio.text,
                  _fechaAlerta.text, _mensajeAlerta.text);
            });
            const SizedBox(
              height: 40,
            );
